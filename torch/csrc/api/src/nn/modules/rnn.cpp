...

  {
    ...
    // an inplace operation on self._flat_weights
    {
      ...
      torch::_cudnn_rnn_flatten_weight(
            ...
            options.bias() ? 4 : 2,
            options.input_size(),
            static_cast<int64_t>(get_cudnn_mode_for_rnn(options.mode())),
            options.hidden_size(),
            options.num_layers(),
            options.batch_first(), 
...
std::tuple<Tensor, Tensor> RNNImplBase<Derived>::forward_helper(
  ...
  if (!batch_sizes.defined()) {
    if (c10::get_if<enumtype::kRNN_TANH>(&options.mode())) {
      result = torch::rnn_tanh(input, hx, _flat_weights, options.bias(), options.num_layers(),
                               options.dropout(), this->is_training(), options.bidirectional(), options.batch_first());
    } else if (c10::get_if<enumtype::kRNN_RELU>(&options.mode())) {
      result = torch::rnn_relu(input, hx, _flat_weights, options.bias(), options.num_layers(),
                               options.dropout(), this->is_training(), options.bidirectional(), options.batch_first());
    } else {
      TORCH_CHECK(false, "Unknown mode: ", torch::enumtype::get_enum_name(options.mode()));
    }
  } else {
    if (c10::get_if<enumtype::kRNN_TANH>(&options.mode())) {
      result = torch::rnn_tanh(input, batch_sizes, hx, _flat_weights, options.bias(),
                               options.num_layers(), options.dropout(), this->is_training(), options.bidirectional());
    } else if (c10::get_if<enumtype::kRNN_RELU>(&options.mode())) {
      result = torch::rnn_relu(input, batch_sizes, hx, _flat_weights, options.bias(),
                               options.num_layers(), options.dropout(), this->is_training(), options.bidirectional());
    } else {
      TORCH_CHECK(false, "Unknown mode: ", torch::enumtype::get_enum_name(options.mode()));
    }
...
template <typename Derived>
std::tuple<PackedSequence, Tensor> RNNImplBase<Derived>::forward_with_packed_input(const PackedSequence& packed_input, Tensor hx) {
