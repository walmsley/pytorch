...
void copyParameters(torch::nn::ModuleHolder<Impl>& target, size_t t_i,
  ...
  for (const auto& item : target->named_parameters()) { // yf225 TODO debug
    std::cout << item.key() << std::endl;
  }
  std::cout << "source: " << std::endl;
  for (const auto& item : source->named_parameters()) { // yf225 TODO debug
    std::cout << item.key() << std::endl;
  }
  target->named_parameters()["weight_ih_l" + c10::str(t_i)].copy_(source->named_parameters()["weight_ih_l" + c10::str(s_i)]);
  target->named_parameters()["weight_hh_l" + c10::str(t_i)].copy_(source->named_parameters()["weight_hh_l" + c10::str(s_i)]);
  target->named_parameters()["bias_ih_l" + c10::str(t_i)].copy_(source->named_parameters()["bias_ih_l" + c10::str(s_i)]);
  target->named_parameters()["bias_hh_l" + c10::str(t_i)].copy_(source->named_parameters()["bias_hh_l" + c10::str(s_i)]);
}
...
void BidirectionalGRUReverseForward(bool cuda) {
  ...
  copyParameters(reverse_gru, 0, bi_grus, 0);
...
void BidirectionalLSTMReverseForwardTest(bool cuda) {
  ...
  // ones of the (reversed) bidirectional's:
  copyParameters(reverse_lstm, 0, bi_lstm, 0);
...
TEST_F(RNNTest, BidirectionalMultilayerGRU_CPU_vs_CUDA) {
  ...
  {
    at::NoGradGuard guard;
    // yf225 TODO: should iterate through named_parameters(), instead of doing nested for-loop over layer_idx
